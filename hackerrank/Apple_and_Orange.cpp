...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    int orange_cnt = 0, apple_cnt = 0;
    int apple_point = a, orange_point = b;
    for(int i=0; i<apples.size(); i++) {
        int tmp = apples[i] + apple_point;
        if(s<=tmp && tmp<=t) apple_cnt++;
    }
    for(int i=0; i<oranges.size(); i++) {
        int tmp = oranges[i] + orange_point;
        if(s<=tmp && tmp<=t) orange_cnt++;
    }
    cout << apple_cnt << "\n" << orange_cnt;
}
